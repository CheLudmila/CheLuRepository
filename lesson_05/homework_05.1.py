...
# Фільтруємо машини по критеріях
filtered_cars = [
    (brand, details)
    for brand, details in car_data.items()
    if details[1] >= search_criteria[0]
    and details[2] >= search_criteria[1]
    and details[4] <= search_criteria[2]
]
...
print("Top 5 cars by search criteria:")
for brand, details in filtered_cars[:5]:
    print(f"{brand}: Year {details[1]}, Engine {details[2]}L, Price ${details[4]}")