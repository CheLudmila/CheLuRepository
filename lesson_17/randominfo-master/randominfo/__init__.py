...
def get_address():
    ...
    next(addrFile)  # пропустити заголовки
    all_addresses = []

    for row in addrFile:
        try:
            address = {
                'street': row[5] if len(row) > 5 else '',
                'landmark': row[6] if len(row) > 6 else '',
                'area': row[7] if len(row) > 7 else '',
                'city': row[8] if len(row) > 8 else '',
                'state': row[9] if len(row) > 9 else '',
                'country': row[10] if len(row) > 10 else '',
                'pincode': row[11] if len(row) > 11 else '',
            }
